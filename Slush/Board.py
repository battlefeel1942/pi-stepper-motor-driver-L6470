...
class sBoard:
  ...
  def initGPIOState(self):
    ...
    #DISABLE WARNINGS
    #Hide warnings - for testing - gpio.cleanup() should be used that the end of each script.
    ...
    gpio.setup(L6470_Reset, gpio.OUT)
    
    #chip select pins, must all be low or SPI will com fail
    gpio.setup(MTR0_ChipSelect, gpio.OUT)
    gpio.setup(MTR1_ChipSelect, gpio.OUT)
    gpio.setup(MTR2_ChipSelect, gpio.OUT)
    gpio.setup(MTR3_ChipSelect, gpio.OUT)
    gpio.setup(MTR4_ChipSelect, gpio.OUT)
    gpio.setup(MTR5_ChipSelect, gpio.OUT)
    gpio.setup(MTR6_ChipSelect, gpio.OUT)
    gpio.output(MTR0_ChipSelect, gpio.HIGH)
    gpio.output(MTR1_ChipSelect, gpio.HIGH)
    gpio.output(MTR2_ChipSelect, gpio.HIGH)
    gpio.output(MTR3_ChipSelect, gpio.HIGH)
    gpio.output(MTR4_ChipSelect, gpio.HIGH)
    gpio.output(MTR5_ChipSelect, gpio.HIGH)
    gpio.output(MTR6_ChipSelect, gpio.HIGH)

    #IO expander reset pin
    gpio.setup(MCP23_Reset, gpio.OUT)
    gpio.output(MCP23_Reset, gpio.HIGH)

    #preforma a hard reset
    gpio.output(L6470_Reset, gpio.LOW)
    time.sleep(.1)
    gpio.output(L6470_Reset, gpio.HIGH)    
    time.sleep(.1)  
